        ...
        stage('Build Services') {
            steps {
                script {
                    try {
                        // 각각의 서비스 디렉토리에서 빌드
                        def services = ['discovery-service', 'gateway-service', 'user-service', 'config-service', 'admin-service', 'auth-service']
        ...
        stage('Build Docker Images') {
            steps {
                script {
                    try {
                        // Docker 이미지 빌드
                        def services = ['discovery-service', 'gateway-service', 'user-service', 'config-service', 'admin-service', 'auth-service', 'fluentd']
                        services.each { service ->
                            dir(service) {
